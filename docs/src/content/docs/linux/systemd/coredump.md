...
| Option | Description |
| --- | --- |
| `Storage` | Where to store core dumps: `"external"` (files), `"journal"`, or `"none"`. |
| `ProcessSizeMax` | Maximum core size to generate stack trace (default: 1G on 32-bit, 32G on 64-bit). |
| `ExternalSizeMax` | Maximum core size to save to external storage (default: 1G on 32-bit, 32G on 64-bit). |
